...
import { Bookmark, Paperclip, MessageSquare, Clock, CheckCircle, Edit, Trash2 } from 'lucide-react';
...
const TaskItem = ({ task, onToggleBookmark, onEditTask, onDeleteTask, onMarkAsDone }) => {
    ...
    return (
        <div className="flex items-center py-3 border-b border-gray-200 last:border-b-0 hover:bg-gray-50 transition-colors duration-150 group relative">
            ...
            {/* Due Date */}
            <div className="w-28 px-2 text-sm text-gray-600">{task.dueDate}</div>

            {/* Remaining Time */}
            <div className={`w-24 px-2 text-sm ${task.overdue ? 'text-red-500' : 'text-gray-600'}`}>{task.remaining}</div>
